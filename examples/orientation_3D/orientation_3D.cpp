...
int main()
{
  ...
  if(Pozyx.begin(7, false, MODE_INTERRUPT, POZYX_INT_MASK_IMU) == POZYX_FAILURE){
...
void loop(){
  ...
  std::cout << "," << (int)pressure;

  // print out all remaining sensors
  for(i=2; i<24; i++){
    std::cout << "," << (int)sensor_data[i];
  }

  // finally, print out the calibration status (remotely this is not available and all equal to zero)
  std::cout << "," << ((int)calib_status&0x03);
  std::cout << "," << (((int)calib_status&0x0C)>>2);
  std::cout << "," << (((int)calib_status&0x30)>>4);
  std::cout << "," << (((int)calib_status&0xC0)>>6) << std::endl;
}
